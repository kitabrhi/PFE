...
// Utilitaires
  ...
  // retourne vrai via callback si le point fort existe déjà, sans lever d'assertion Cypress
  private static pointFortExiste(version: Version, texte: string, callback: (existe: boolean) => void): void {
    ...
    if (version === 'v1') {
      ...
      cy.get('body').then($body => {
        callback($body.find(`${rowSelector}:contains("${texte}")`).length > 0);
      });
    }
  }
  ...
  static supprimerPointFort(version: Version, texte: string): void {
    ...
    if (version === 'v1') {
      ...
      cy.get('@lignePointFort').scrollIntoView().within(() => {
        cy.get(getSelector(SECTION_POINTS_FORTS.BTN_MENU_CONTEXTUEL, version)).click();
      });
      PointsFortsPrimitives.attendreAutoSave();
    }
  }
